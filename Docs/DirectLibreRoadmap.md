...
The user subsequently passed the remaining requested restart, signal-loss,
...
5. **Phone-controlled switching.** Persist selected device, session identifier,
   ...
   activate Watch after xDrip confirms its local phone connection is released.
   Return to phone likewise waits for confirmed local Watch release. Reserve/persist
   counters before unlock writes. Reject obsolete transactions. A successful ordinary NFC
