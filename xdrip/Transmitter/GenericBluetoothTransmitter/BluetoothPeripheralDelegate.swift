...
/// delegate used for any type of bluetoothPeripheral, some functions are not used for some types
protocol BluetoothPeripheralDelegate: AnyObject {
 
    // MARK: - Generic functions that can be used for any type of BluetoothPeripheral
    
    /// did connect to
    /// - parameters:
    ///     - forBluetoothPeripheral : if nil then it's the first connection to a new BluetoothPeripheral, time to create a new BluetoothPeripheral instance with address and string which should now not be nil
    ...
    /// did disconnect from BluetoothPeripheral
    ...
    /// will be called if BluetoothPeripheral is connected and ready to receive data
    func isReadyToReceiveData(bluetoothPeripheral : BluetoothPeripheral)
    
    /// bluetoothPeripheral wants to receive all parameters
    func isAskingForAllParameters(bluetoothPeripheral: BluetoothPeripheral)
    
    // MARK: - M5Stack specific functions
