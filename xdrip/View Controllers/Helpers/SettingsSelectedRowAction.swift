...
enum SettingsSelectedRowAction {
	...
	/// takes as argument a callback that when called returns an optional URL,  that if not nil the user should be presented with a share menu.
    /// - Important: Displays a loading indicator, so all routies MUST call the callback or will result in an endless loading indicator.
