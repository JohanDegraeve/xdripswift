...
enum ConstantsWidgetExtension {
    ...
    static let bgReadingDateStaleInMinutes = 7.0 * 60
    
    /// the time in minutes until the last BG value is considered too old to show
    static let bgReadingDateVeryStaleInMinutes = 20.0 * 60
    
    /// from what time we will consider that the widget is being displayed at night
    static let nightModeFromHour: Int = 22 // 22:00hrs / 10pm
    
    /// until what time we will consider that the widget is being displayed at night
    static let nightModeUntilHour: Int = 8 // 082:00hrs / 8am
