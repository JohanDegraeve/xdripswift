...
Create a structure containing uECC_HashContext followed by hash-specific data.
For SHA-256, include a SHA256_CTX member. Each callback casts the base pointer
back to the containing structure. The init callback calls SHA256_Init, the update
callback passes the message and its size to SHA256_Update, and the finish callback
passes the output buffer to SHA256_Final.

Set block_size to 64 and result_size to 32. Supply a temporary buffer of
32 + 32 + 64 bytes, assign the three callbacks, and pass the embedded
uECC_HashContext to uECC_sign_deterministic().
*/
