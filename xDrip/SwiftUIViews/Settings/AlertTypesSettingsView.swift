...
final class AlertTypesSettingsHostingController: PortraitLockedHostingController<AnyView> {
    private let viewModel: AlertTypesSettingsViewModel
    private let coreDataManager: CoreDataManager
    private let soundPlayer: SoundPlayer

    /// Hosts the SwiftUI alert type list from UIKit so it can be pushed by the
    /// main Settings navigation controller.
    init(coreDataManager: CoreDataManager, soundPlayer: SoundPlayer) {
        let viewModel = AlertTypesSettingsViewModel(coreDataManager: coreDataManager)

        self.viewModel = viewModel
        self.coreDataManager = coreDataManager
        self.soundPlayer = soundPlayer

        super.init(rootView: AnyView(AlertTypesSettingsView(viewModel: viewModel) { _ in }))

        rootView = AnyView(AlertTypesSettingsView(viewModel: viewModel) { [weak self] alertType in
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // Reload here because returning from an editor should immediately update
        // the list, even if the Core Data object changed outside this view model.
        viewModel.reload()
    }

    /// Pushes the SwiftUI alert type editor and reloads the list when it closes.
    /// Passing nil starts the add flow; passing an AlertType edits an existing one.
    private func open(alertType: AlertType?) {
        let viewController = PortraitLockedHostingController(
            rootView: AlertTypeEditorView(
                alertType: alertType,
                coreDataManager: coreDataManager,
                soundPlayer: soundPlayer,
                close: { [weak self] in
        ...
        viewController.title = Texts_AlertTypeSettingsView.editAlertTypeScreenTitle
