...
final class AlertsSettingsHostingController: PortraitLockedHostingController<AnyView> {
    private let viewModel: AlertsSettingsViewModel
    private let coreDataManager: CoreDataManager

    /// Hosts the SwiftUI alarm list from UIKit so the main Settings flow can push it
    /// in the same way the old storyboard segue did.
    init(coreDataManager: CoreDataManager) {
        let viewModel = AlertsSettingsViewModel(coreDataManager: coreDataManager)

        self.viewModel = viewModel
        self.coreDataManager = coreDataManager

        super.init(rootView: AnyView(AlertsSettingsView(viewModel: viewModel) { _ in }))

        rootView = AnyView(AlertsSettingsView(viewModel: viewModel) { [weak self] request in
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // Refresh whenever the list becomes visible again because alarm editors
        // write directly to Core Data.
        viewModel.reload()
    }

    /// Opens the editor for an existing alarm using the start-time limits calculated
    /// by the list view model.
    private func open(request: AlertEntryEditRequest) {
        open(mode: .edit(
    ...
    /// Pushes an alarm editor for either edit or add mode.
    /// Closing any editor returns to the alarm list and refreshes the Core Data rows.
    private func open(mode: AlertEntryEditorMode) {
        let viewController = PortraitLockedHostingController(
            rootView: AlertEntryEditorView(
                mode: mode,
                coreDataManager: coreDataManager,
                close: { [weak self] in
                    guard let self else { return }
                ...
                openNewAlert: { [weak self] newMode in
                    self?.open(mode: newMode)
                }
            )
