...
class M5StackPacket {
    ...
    func getText() -> String? {
        ...
        guard timeStampLastPacket != nil,
              let totalNumberOfPacketsToReceive = totalNumberOfPacketsToReceive,
              packetsReceived.count == totalNumberOfPacketsToReceive else {return nil}
    ...
    func addNewPacket(value: Data) {
        ...
        let totalNumberOfPackets = Int(value.uint8(position: 2))
        let numberOfThisPacket = Int(value.uint8(position: 1))
        guard totalNumberOfPackets > 0,
              numberOfThisPacket > 0,
              numberOfThisPacket <= totalNumberOfPackets else {return}
        ...
        // a different packet count belongs to a new message, even when it arrives before the timeout
        if let totalNumberOfPacketsToReceive = totalNumberOfPacketsToReceive, totalNumberOfPacketsToReceive != totalNumberOfPackets {
            resetAllProperties()
        }
        ...
        totalNumberOfPacketsToReceive = totalNumberOfPackets
        ...
        if packetsReceived.count == 0 {
            ...
            packetsReceived = Array(repeating: Data(), count: totalNumberOfPackets)
        }
